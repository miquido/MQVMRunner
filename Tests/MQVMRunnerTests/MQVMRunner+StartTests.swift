...
final class MQVMRunnerStartTests: XCTestCase {
    func testExecute() async throws {
        ...
        var command = command(
            commandExecutor: .mock(resultHandler: { _ in
                "Xcode_16.0"
            }, {
            ...
            sshExecutor: .mock(execute: {
                ...
                return AsyncThrowingStream { continuation in
                    continuation.yield(ExecCommandOutput.stdout(ByteBuffer(string: "Xcode_16.0")))
                    continuation.finish()
                }
            })
        )
        ...
        await fulfillment(of: [cloneExpectation,
                               runExpectation,
                               ipExpectation,
                               sshTestExpectation,
    ...
    func testCloneFail() async throws {
        let command = command(commandExecutor: .mock({
            if $0.command.hasSuffix("tart"), $0.string.contains("clone") {
                throw CloneError()
            }
        }))
